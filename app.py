...
class SimpleApp(QWidget):
    ...
    def prikazi_prosireni_turnus(self):
        ...
        # Pretpostavljamo da self.scene i self.view postoje iz create_tab_grafik
        ...
        if hasattr(self, 'tabs'):
            # Pretpostavljamo da je tab "Pregled Grafika" na indeksu 2 (0-Vozovi, 1-Turnusi, 2-Grafik)
            # Proveri tačan indeks u svom kodu!
            self.tabs.setCurrentIndex(2)  # <- PROMENJENO SA 3 NA 2
    ...
    def _crtaj_jedan_turnus_prosireni(self, vozovi, y, sirina_sata, visina_turnusa, pocetni_sat, krajnji_sat):
        ...
        # 1. Broj vučnog vozila (1) - uvek na istom mestu
        self.stampa_scene.addText("1").setPos(broj_vozila_x, broj_vozila_y)

        # 2. Glavna horizontalna linija puta turnusa - ograničena na zadati opseg
        # Početak i kraj u pikselima
        x_pocetak = (pocetni_sat - 0) * sirina_sata
        x_kraj = (krajnji_sat - 0) * sirina_sata
        self.stampa_scene.addLine(x_pocetak, gornja_linija_y, x_kraj, gornja_linija_y, QPen(Qt.GlobalColor.black, 1.2))
        self.stampa_scene.addLine(x_pocetak, donja_linija_y, x_kraj, donja_linija_y, QPen(Qt.GlobalColor.black, 1.2))

        # 3. Vertikalne podelice na glavnoj liniji - samo za zadati opseg
        for sat in range(pocetni_sat, krajnji_sat + 1):  # Uključuje i krajnji_sat za crtanje poslednje linije
            x = (sat - 0) * sirina_sata
            # Kratka vertikalna podelica: dužina ~6px (≈2mm) na [3], ~6px (≈2mm) na [5]
            self.stampa_scene.addLine(x, gornja_linija_y - 3, x, gornja_linija_y + 3, QPen(Qt.GlobalColor.black, 0.8))
            self.stampa_scene.addLine(x, donja_linija_y - 3, x, donja_linija_y + 3, QPen(Qt.GlobalColor.black, 0.8))

            # Broj sata iznad gornje linije (samo unutar opsega)
            if sat < krajnji_sat:  # Ne crtamo broj posle poslednje podelice (24:00 npr.)
                sat_tekst = f"{sat:02d}"
                text = self.stampa_scene.addText(sat_tekst)
                text.setFont(QFont("Arial", 8))
                text.setPos(x - 10, -30)  # Ispod glavne vremenske ose (na y=0)
        ...
        # 5. Crtanje svakog voza u turnusu
        for i, voz in enumerate(vozovi):
            # a) Izračunaj x koordinate za polazak i dolazak u odnosu na ceo dan (0-24)
            # Ovo je ključno jer se x koordinate uvek računaju u odnosu na 00:00 celog dana
            x_p = (voz['polazak'][0] * 60 + voz['polazak'][1]) / 60 * sirina_sata
            x_d = (voz['dolazak'][0] * 60 + voz['dolazak'][1]) / 60 * sirina_sata

            # b) Linija vožnje (horizontalna ispod gornje linije)
            linija_y = gornja_linija_y + 10
            pen.setStyle(style_map.get(voz['status'], Qt.PenStyle.SolidLine))
            # Crtamo liniju samo ako se bar deo njenog x opsega poklapa sa opsegom ovog "reda"
            # Jednostavnija provera: da li je bilo koji kraj unutar opsega ili da linija obuhvata ceo opseg
            opseg_x_pocetak = x_pocetak
            opseg_x_kraj = x_kraj
            ...
            if not (x_p > opseg_x_kraj or x_d < opseg_x_pocetak):  # Standardna provera preseka intervala
                # Postoji presek, crtamo liniju
                # Ali crtamo samo deo koji pripada trenutnom opsegu
                clipped_x1 = max(x_p, opseg_x_pocetak)
                clipped_x2 = min(x_d, opseg_x_kraj)
                self.stampa_scene.addLine(clipped_x1, linija_y, clipped_x2, linija_y, pen)

            # c) Broj voza (centrirano između x_p i x_d, ali samo ako je unutar opsega)
            if not (x_p > opseg_x_kraj or x_d < opseg_x_pocetak):
                clipped_x1 = max(x_p, opseg_x_pocetak)
                clipped_x2 = min(x_d, opseg_x_kraj)
                text_broj = self.stampa_scene.addText(voz['broj'])
                text_broj.setPos((clipped_x1 + clipped_x2) / 2 - 20, tekst_gore_y)

            # d) Minuti (pored x_p i x_d, ali samo ako su unutar opsega)
            # Za početak: proveri da li je x_p unutar opsega
            if opseg_x_pocetak <= x_p <= opseg_x_kraj:
                text_min_p = self.stampa_scene.addText(f"{voz['polazak'][1]:02}")  # minuti
                text_min_p.setPos(x_p - 10, tekst_dole_y)
            # Za dolazak: proveri da li je x_d unutar opsega
            if opseg_x_pocetak <= x_d <= opseg_x_kraj:
                text_min_d = self.stampa_scene.addText(f"{voz['dolazak'][1]:02}")  # minuti
                text_min_d.setPos(x_d - 10, tekst_dole_y)

            # e) Stanice (samo za prvi i poslednji voz u ovom DELU turnusa)
            if i == 0:  # Prvi voz u ovom delu
                # Proveri da li je x_p unutar opsega da bi crtali stanicu
                if opseg_x_pocetak <= x_p <= opseg_x_kraj:
                    text_pocetna = self.stampa_scene.addText(voz['pocetna'])
                    text_pocetna.setPos(x_p - 20, tekst_gore_y - 15)
            if i == len(vozovi) - 1:  # Poslednji voz u ovom delu
                # Proveri da li je x_d unutar opsega da bi crtali stanicu
                if opseg_x_pocetak <= x_d <= opseg_x_kraj:
                    text_krajnja = self.stampa_scene.addText(voz['krajnja'])
                    text_krajnja.setPos(x_d - 20, tekst_gore_y - 15)
            else:
                # Srednji vozi - stanica dolaska trenutnog = stanica polaska sledećeg
                # crtamo između x_d trenutnog i x_p sledećeg, ako je unutar opsega
                sledeci = vozovi[i + 1]
                x_d_trenutni = x_d
                x_p_sledeci = (sledeci['polazak'][0] * 60 + sledeci['polazak'][1]) / 60 * sirina_sata

                # x_sredina je tačka između dolaska trenutnog i polaska sledećeg
                # Ali crtamo samo ako je ta tačka unutar opsega
                x_sredina = (x_d_trenutni + x_p_sledeci) / 2
                if opseg_x_pocetak <= x_sredina <= opseg_x_kraj:
                    # Provera da li su oba kraja unutar opsega za crtanje linije između njih
                    # (opciono, da linija ne ide van okvira)
                    clipped_x1 = max(x_d_trenutni, opseg_x_pocetak)
                    clipped_x2 = min(x_p_sledeci, opseg_x_kraj)

                    text_srednja = self.stampa_scene.addText(voz['krajnja'])  # Stanica dolaska trenutnog
                    text_srednja.setPos(x_sredina - 20, tekst_gore_y - 15)  # Pozicioniramo na sredinu
