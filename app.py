...
class SimpleApp(QWidget):
    def __init__(self):
        ...
        # Inicijalizacija UI - OVO MORA BITI PRE populate_filters_and_load_data
        self.init_ui()

        # Popunjavanje filtera i učitavanje podataka - OVO IDE POSLE init_ui
    ...
    def create_tab_grafik(self):
        ...
        # TODO: Povezati sa odgovarajucom funkcijom
        self.btn_prikazi_prosireni.clicked.connect(self.prikazi_prosireni_turnus)  # <- DODATO
    ...
    def create_tab_stampa(self):
        ...
        left_top_layout.addWidget(QLabel("Kontrole za izbor turnusa"))
        # TODO: Dodati kontrole za izbor sekcije, filtriranje turnusa itd.
        # Za sada je prazan placeholder
        left_top_layout.addStretch()
        top_layout.addWidget(left_top_frame, 33)  # 33% širine
        ...
        middle_top_layout.addWidget(QLabel("Pregled selekcije"))
        # TODO: Dodati widget za prikaz izabranih turnusa
        # Za sada je prazan placeholder
        middle_top_layout.addStretch()
        top_layout.addWidget(middle_top_frame, 33)  # 33% širine

        # === GRAFIČKI TURNUS VUČNIH VOZILA ZA RED VOŽNJE (33.33%) ===
        right_top_frame = QFrame()
        right_top_frame.setFrameShape(QFrame.Shape.StyledPanel)
        right_top_layout = QVBoxLayout(right_top_frame)  # Koristi QVBoxLayout za vertikalni raspored
        ...
        right_top_layout.addWidget(naslov_label)

        # Horizontalni layout za input i label "GODINU"
        input_layout = QHBoxLayout()
        input_layout.addStretch()  # Dodaj fleksibilni prostor levo

        # QLineEdit za unos godine - KORISTIMO ISTU INSTANCU KAO I ZA GRAFIK
        # Proveravamo da li self.godina_input već postoji (iz create_tab_grafik)
        # Ako ne postoji, kreiraćemo je ovde, ali bolje je da postoji iz create_tab_grafik
        # Pretpostavimo da self.godina_input postoji. Ako ne, dodajemo sigurnosnu mreu.
        if not hasattr(self, 'godina_input') or self.godina_input is None:
            # Sigurnosna mera ako nije inicijalizovana ranije (npr. ako se tab "Stampa" kreira pre "Grafik")
            self.godina_input = QLineEdit()
            self.godina_input.setPlaceholderText("Npr. 09.12.2025/26")
            # self.godina_input.setText(self.godina_za_grafik) # Ovo se poziva u ucitaj_godinu_za_grafik
            self.godina_input.textChanged.connect(self.snimi_godinu_za_grafik)

        # PODEBLJAJ FONT I CENTRIRAJ TEKST U QLineEdit
        font_le = self.godina_input.font()
        font_le.setBold(True)
        self.godina_input.setFont(font_le)
        self.godina_input.setAlignment(Qt.AlignmentFlag.AlignCenter)

        # UCITAJ VREDNOST IZ FAJLA (ako vec nije ucitana u init_database ili create_tab_grafik)
        # Ovo bi trebalo da se desi jednom pri inicijalizaciji, ali za svaki slucaj proverimo
        # self.ucitaj_godinu_za_grafik() # Ovo vec treba da je pozvano u init_database
        # Ako nije, mozemo ga pozvati ovde, ali bolje je osigurati da se u init_database pozove jednom
        # i da self.godina_za_grafik bude inicijalizovan

        input_layout.addWidget(self.godina_input)
        input_layout.addStretch()  # Dodaj fleksibilni prostor desno
        right_top_layout.addLayout(input_layout)  # Dodaj horizontalni layout

        # QLabel "GODINU" ISPOT QLineEdiTA - PODEBLJAJ I CENTRIRAJ
        label_godina = QLabel("GODINU")
        label_godina.setAlignment(Qt.AlignmentFlag.AlignCenter)  # Centriraj tekst
        label_godina.setStyleSheet("font-weight: bold;")  # Podebljaj tekst
        right_top_layout.addWidget(label_godina)  # Dodaj ispod inputa

        top_layout.addWidget(right_top_frame, 34)  # 34% širine (zaokruženo)

        main_layout.addWidget(top_frame, 25)  # 25% visine
        ...
        main_layout.addWidget(bottom_frame, 75)  # 75% visine
    ...
    def populate_sekcije_turnusi_filter(self):
        ...
        for s in sekcije:
            ...
            cb.stateChanged.connect(
                lambda state, checkbox=cb: self.on_individual_checkbox_changed(
                    state, checkbox, self.all_sekcije_turnusi_cb,
                    lambda: self.ucitaj_turnuse(sort_column=None, sort_order=None)
                )
            )
    ...
    def populate_serije_vv_filter(self):
        ...
        for s in serije:
            ...
            cb.stateChanged.connect(
                lambda state, checkbox=cb: self.on_individual_checkbox_changed(
                    state, checkbox, self.all_serije_vv_cb,
                    lambda: self.ucitaj_turnuse(sort_column=None, sort_order=None)
                )
            )
    ...
    def proveri_turnus(self):
        ...
        if not greske and len(vozovi) > 1:
            ...
            if poslednji_voz_broj in vozovi_info and prvi_voz_broj in vozovi_info:
                ...
                prelazni_poslednji = (sat_d_poslednji < sat_p_poslednji) or (
                            sat_d_poslednji == sat_p_poslednji and min_d_poslednji < min_p_poslednji)
    ...
    def prikazi_prosireni_turnus(self):
        ...
        sirina_sata = 60  # Ovo je širina sata za prikaz (npr. 60px po satu)
        visina_reda = 120
        y_pocetak = 50
        ...
        if deo1_pre_podne:
            y_deo1 = y_pocetak
            # Poziv funkcije sa dodatnim argumentima za početni i krajnji sat
        ...
        # Širina scene: 24 sata * širina sata
    ...
    def _crtaj_jedan_turnus_prosireni(self, vozovi, y_pocetak, sirina_sata, visina_reda, pocetni_sat, krajnji_sat):
        """
        ...
        # Broj vucnog vozila (1)
        self.scene.addText("1").setPos(broj_vozila_x, broj_vozila_y)
        # Glavna linija puta turnusa - ograničena na zadati opseg
        ...
        self.scene.addLine(pocetni_sat * sirina_sata, donja_linija_y, krajnji_sat * sirina_sata, donja_linija_y,
                           QPen(Qt.GlobalColor.black, 1.2))
        # Vertikalne podelice na linijama puta - samo za zadati opseg
        for sat in range(pocetni_sat, krajnji_sat + 1):
            x = sat * sirina_sata
            # Kratka vertikalna podelica: dužina ~6px (≈2mm) na [3], ~6px (≈2mm) na [5]
            self.scene.addLine(x, gornja_linija_y - 3, x, gornja_linija_y + 3, QPen(Qt.GlobalColor.black, 0.8))
            self.scene.addLine(x, donja_linija_y - 3, x, donja_linija_y + 3, QPen(Qt.GlobalColor.black, 0.8))
            # Broj sata iznad gornje linije [3] - samo ako nije krajnji sat (24:00 npr.)
            if sat < krajnji_sat:
                sat_tekst = f"{sat:02d}"
                text = self.scene.addText(sat_tekst)
                text.setFont(QFont("Arial", 8))
                text.setPos(x - 10, tekst_gore_y - 15)  # Malo više iznad linije
        ...
        for i, voz in enumerate(vozovi):
            # Izračunaj x koordinate za polazak i dolazak u odnosu na ceo dan (0-24)
            x_p = (voz['polazak'][0] * 60 + voz['polazak'][1]) / 60 * sirina_sata
            x_d = (voz['dolazak'][0] * 60 + voz['dolazak'][1]) / 60 * sirina_sata

            # Linija vožnje - ograničena na zadati opseg
            # Provera da li linija ima presek sa trenutnim opsegom crtanja
            ...
            if not (x_p > opseg_x_kraj or x_d < opseg_x_pocetak):  # Standardna provera preseka intervala
                ...
                # Broj voza - crtaj samo jednom za ceo voz, na sredini segmenta koji se crta
                # Ovo rešava problem "neprikazanih brojeva vozova"
                text_broj = self.scene.addText(voz['broj'])
                text_broj.setPos((clipped_x1 + clipped_x2) / 2 - 20, tekst_gore_y)

                # Minuti - crtaj na početku i kraju segmenta koji se crta
                # Ovo rešava problem "neprikazanih minuta"
                # Minut polaska (na početku segmenta koji se crta)
                if opseg_x_pocetak <= x_p <= opseg_x_kraj:
                    text_min_p = self.scene.addText(f"{voz['polazak'][1]:02d}")
                    text_min_p.setPos(x_p - 10, tekst_dole_y)
                # Minut dolaska (na kraju segmenta koji se crta)
                if opseg_x_pocetak <= x_d <= opseg_x_kraj:
                    text_min_d = self.scene.addText(f"{voz['dolazak'][1]:02d}")
                    text_min_d.setPos(x_d - 10, tekst_dole_y)

                # Stanice - crtaj samo jednom, na mestima gde se stvarno nalaze
                # Ovo rešava problem "neprikazanih stanica"
                if i == 0:  # Prvi voz u celom turnusu (ako je u ovom opsegu)
                    # Crtaj početnu stanicu ako je njen x_p u ovom opsegu
                    if opseg_x_pocetak <= x_p <= opseg_x_kraj:
                        text_pocetna = self.scene.addText(voz['pocetna'])
                        text_pocetna.setPos(x_p - 20, tekst_gore_y - 15)
                # Crtaj krajnju stanicu svakog voza ako je njen x_d u ovom opsegu
                # Ovo rešava problem "neprikazanih stanica u sredini i na kraju"
                if opseg_x_pocetak <= x_d <= opseg_x_kraj:
                    text_krajnja = self.scene.addText(voz['krajnja'])
                    text_krajnja.setPos(x_d - 20, tekst_gore_y - 15)
                # Napomena: "Srednje" stanice (između vozova) se crtaju kao krajnje stanice prethodnog voza.
                # Ako voz 1111 stigne u PO, i voz 2222 polazi iz PO, stanica PO će biti nacrtana
                # kao krajnja stanica vozа 1111 (gore) i kao početna stanica vozа 2222 (dole),
                # ali pošto se crtaju samo ako su u opsegu, neće biti duplikata ako su u različitim redovima.

    def _crtaj_segment_voznje(self, voz, x1_cr, x2_cr, y_base_red, sirina_sata_dupla, tip_segmenta, minuti_abs_p,
                              minuti_abs_d):
        """
        ...
        # potreban je dodatni kontekst (da li je voz prvi/poslednji u turnusu).
        # Za sada, crtamo stanice za svaki segment.
        if "segment1" in tip_segmenta or tip_segmenta in ["gornji", "donji"]:
            # Na početku segmenta (ako je početak celog voza)
            text_pocetna = self.scene.addText(voz['pocetna'])
            text_pocetna.setPos(x1_cr - 20, tekst_gore_y - 15)

        if "segment2" in tip_segmenta or tip_segmenta in ["gornji", "donji"]:
            # Na kraju segmenta (ako je kraj celog voza)
            text_krajnja = self.scene.addText(voz['krajnja'])
            text_krajnja.setPos(x2_cr - 20, tekst_gore_y - 15)

        # Napomena: Ova logika za stanice može dovesti do duplikata ako voz ima više segmenata.
        # Tačnije rešenje bi zahtevalo da funkcija zna da li je segment početak/kraj celog voza
        # i da li je voz prvi/poslednji u celom turnusu. To bi zahtevalo promenu u
        # `prikazi_prosireni_turnus` da prošalje dodatne informacije.
        # Za sada, ovo rešenje bi trebalo da bude dovoljno jasno za većinu slučajeva.


    def _crtaj_segment_voznje(self, voz, x1_cr, x2_cr, y_base_red, sirina_sata_dupla, tip_segmenta, minuti_abs_p, minuti_abs_d):
        """
        Pomoćna funkcija za crtanje jednog segmenta linije vožnje.
        x1_cr, x2_cr: x koordinate segmenta (već transformisane u dupli razmak).
        y_base_red: y koordinata gornje ivice celog reda.
    ...
    def _crtaj_elemente_voznje(self, voz, x1_cr, x2_cr, y_base_red, minuti_abs_p, minuti_abs_d, segment_tip):
        """
        Pomoćna funkcija za crtanje broja voza, minuta i stanica za jedan segment.
        x1_cr, x2_cr: x koordinate segmenta (već transformisane).
        y_base_red: y koordinata gornje ivice celog reda.
        minuti_abs_p, minuti_abs_d: Apsolutno vreme polaska/dolaska segmenta u minutima od ponoći dana 1.
        segment_tip: String koji označava tip segmenta ("A", "B", "A-segment1", "B-segment2", itd.)
                     Koristi se da se izbegne duplikat crtanjem elemenata.
        """
        gornja_linija_y = y_base_red + 30
        donja_linija_y = gornja_linija_y + 20
        tekst_gore_y = gornja_linija_y - 25
        ...
        # Broj voza - crtaj samo za glavni segment (npr. "A" ili "B")
        if segment_tip in ["A", "B"]:
            text_broj = self.scene.addText(voz['broj'])
            text_broj.setPos((x1_cr + x2_cr) / 2 - 20, tekst_gore_y)

        # Minuti - crtaj samo na početku i kraju originalne vožnje
        # Ovo znači da se minuti crtaju samo na segmentima koji predstavljaju
        # apsolutni početak ili kraj vožnje.
        # segment_tip sadrži informaciju da li je segment početni ili krajnji.
        # Na primer, za voz 11:00-13:15:
        # Segment 1 (11:00-12:00) ima segment_tip "A-segment1". Treba nacrtati minut polaska (11:00).
        # Segment 2 (12:00-13:15) ima segment_tip "B-segment2". Treba nacrtati minut dolaska (13:15).
        # Za voz 23:00-02:15:
        # Segment 1 (23:00-24:00) ima segment_tip "B-segment1-prelazni". Treba nacrtati minut polaska (23:00).
        # Segment 2 (00:00-02:15) ima segment_tip "A-segment2-prelazni". Treba nacrtati minut dolaska (02:15).

        if "segment1" in segment_tip or "segment1-prelazni" in segment_tip:
            # Ovo je početak originalne vožnje
            sat_p_orig, min_p_orig = divmod(minuti_abs_p, 60)
            # x_p_orig_cr je x1_cr segmenta
            x_p_orig_cr = x1_cr
            text_min_p = self.scene.addText(f"{min_p_orig:02d}")
            text_min_p.setPos(x_p_orig_cr - 10, tekst_dole_y)

        if "segment2" in segment_tip or "segment2-prelazni" in segment_tip:
            # Ovo je kraj originalne vožnje
            sat_d_orig, min_d_orig = divmod(minuti_abs_d, 60)
            # x_d_orig_cr je x2_cr segmenta
            x_d_orig_cr = x2_cr
            text_min_d = self.scene.addText(f"{min_d_orig:02d}")
            text_min_d.setPos(x_d_orig_cr - 10, tekst_dole_y)

        # Stanice
        # crtaj_stanicu_pocetak i crtaj_stanicu_kraj su boolean koji se postavljaju
        # u glavnoj petlji funkcije `_crtaj_jedan_turnus_prosireni` na osnovu toga
        # da li je voz prvi/poslednji u celom turnusu. Ovde ih ne možemo lako odrediti
        # jer funkcija ne zna redosled vozova u celom turnusu.
        # Zbog toga, za sada ćemo pretpostaviti da se stanice crtaju na svakom segmentu
        # gde je to logično, ali to može dovesti do duplikata.
        # Bolje rešenje bi bilo proslediti dodatne informacije iz glavne funkcije.
        # Na primer, voz može imati atribute `je_prvi_u_turnusu` i `je_poslednji_u_turnusu`.
        ...
        # i da odredi koji je prvi/poslednji.

        # Ako je segment "A" ili "B" (glavni segment), crtaj stanice ako su na krajevima reda?
        # Ovo je heuristika i može biti neprecizno.
        # if segment_tip == "A":
        #     # Ako je x1_cr blizu 0, možda je početak puta
        #     # Ako je x2_cr blizu kraja (12*sirina_sata_dupla), možda je kraj puta
        #     pass
        # if segment_tip == "B":
        #     # Slično
        #     pass

        # Za sada, ne crtamo stanice u ovoj pomoćnoj funkciji, već ćemo ih obraditi u glavnoj funkciji
        # kada imamo više konteksta.
